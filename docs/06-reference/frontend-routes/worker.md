...
`(app)/layout.tsx` centraliza: gate de sessão (`useRequireAuth`), busca do perfil uma vez (`WorkerProfileProvider`), redirecionamento pra completar KYC se faltar documento/selfie/documento de responsável (cobre login por e-mail e por Google), banner persistente (`VerificationBanner`, logo abaixo do Topbar, em qualquer tela) enquanto `kycStatus !== 'approved'`, e polling de notificações (candidaturas + turnos) a cada 60s.
