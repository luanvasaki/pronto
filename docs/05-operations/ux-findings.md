...
## ✅ Resolvido: primeira varredura com conferência visual ao vivo de verdade (login → cadastro → publicar vaga → candidatar-se)

Toda varredura anterior registrou a mesma limitação repetida: "sem conferência visual ao vivo — fica atrás de login, sem credencial de teste documentada". Essa varredura resolveu isso subindo o ambiente local (Postgres + backend + worker + business), registrando contas de teste reais pelo fluxo público (`POST /auth/register`), completando o cadastro de worker (perfil, upload de documento/selfie com imagens de teste geradas localmente, aceite de termos) e de business (CNPJ, upload de documento) inteiramente pela UI, e usando `UPDATE` direto no Postgres **local** (nunca produção) pra aprovar o KYC de ambos e destravar publicação/candidatura — o mesmo mecanismo que `docs/04-development/getting-started.md` já descreve pra dado de seed. Documentando aqui o caminho pra próxima vez não redescobrir do zero:

1. `./scripts/setup-local-db.sh && cd apps/backend && npm run db:migrate && npm run db:seed && npm run db:seed-demo`
2. Subir `apps/backend` (porta 4000), `apps/worker` (3000) e `apps/business` (3200) com `npm run dev` em cada um.
3. Registrar conta de teste: `curl -X POST http://localhost:4000/auth/register -H "Content-Type: application/json" -d '{"email":"...","password":"..."}'`
4. Completar o cadastro pela UI normalmente (o upload de documento/selfie aceita qualquer imagem válida — não precisa ser uma foto real).
5. Aprovar KYC pra destravar o fluxo completo (só em banco local): `UPDATE worker_profiles SET kyc_status='approved' WHERE user_id='...'` e `UPDATE companies SET verification_status='approved' WHERE id='...'`.

Com isso, foi possível ver e confirmar ao vivo (não só ler código): tela de login, tela principal do worker (`/inicio`) vazia e populada, o cadastro completo do worker passo a passo, o dashboard do business (`/painel`), o formulário de publicar vaga (`/vagas/nova`) do preenchimento à publicação real, a vaga aparecendo na lista de Escalas do business, a mesma vaga aparecendo no feed do worker, e o estado "Candidatura enviada" depois de se candidatar de verdade. Achados encontrados só por causa dessa conferência ao vivo (não visíveis lendo o código isoladamente): dois bugs reais corrigidos nesta mesma tarefa — ver seções seguintes.

### Bug corrigido: reaceite de termos mandava trabalhador com cadastro completo de volta pro formulário em branco

`apps/worker/src/app/cadastro/termos/page.tsx` — depois de aceitar uma nova versão do termo, `handleAdvance()` sempre fazia `router.push('/cadastro')` (o formulário de perfil), sem checar se a pessoa já tinha perfil/documentos completos. Isso é inofensivo pra quem está no cadastro inicial (é o próximo passo mesmo), mas pra qualquer trabalhador **já aprovado**, pego pelo gate de `(app)/layout.tsx` só porque uma nova versão do `platform_terms` saiu, o resultado era ser jogado de volta num "Complete seu cadastro" completamente vazio — nome, CPF, endereço, data de nascimento, categoria, tudo pra preencher de novo, mesmo já tendo enviado tudo isso antes. Só foi percebido testando o fluxo de verdade (o comentário no topo do arquivo já previa esse cenário, mas a implementação não cobria o redirecionamento). Corrigido: agora reconsulta `getWorkerProfile()` depois do aceite e decide entre `/inicio`, `/cadastro/documento` ou `/cadastro` conforme o que realmente falta.

### Bug corrigido: tela principal do worker virava beco sem saída em qualquer falha ao carregar vagas

`apps/worker/src/app/(app)/inicio/page.tsx` — o estado de erro (`fetchNearbyJobs()` falhando, o que inclui o caso comum de geolocalização negada/indisponível) renderizava só uma frase vermelha centralizada numa página em branco, sem nenhuma ação possível — só recarregar o app manualmente resolvia. Reproduzido ao vivo (não um cenário hipotético): a chamada de geolocalização do navegador falhou na primeira tentativa desta sessão. Adicionado botão "Tentar novamente" que reexecuta o carregamento; a mensagem também deixou de usar a cor de erro (`text-danger`) já que geolocalização pendente não é uma falha crítica, é um estado recuperável comum.
