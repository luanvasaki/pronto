...
**Renovação no client (`packages/shared/src/api.ts`)**: `apiFetch` tem um interceptor global — qualquer 401 (exceto quando o chamador passa `skipAuthRetry: true`) dispara `refreshSession()` e repete a chamada original uma vez antes de propagar o erro. `skipAuthRetry: true` é usado só nos próprios endpoints de auth onde 401 é resposta de negócio, não sessão expirada (login, register, googleLogin, forgotPassword, resetPassword, refreshSession, logout). Além disso, `useRequireAuth` (duplicado idêntico em worker/business/admin) mantém um `setInterval` de 10 minutos chamando `refreshSession()` proativamente enquanto o layout autenticado fica montado — necessário porque esse layout só monta uma vez por carregamento de página (o App Router não remonta em navegação interna), então sem essa renovação periódica uma sessão longa e contínua esbarraria no access token (15min) vencido no meio do uso. Essas duas camadas — retry reativo no `apiFetch` e renovação proativa no hook — são o que faz o usuário permanecer logado por até 30 dias (validade do refresh token) sem precisar relogar manualmente, similar ao comportamento de apps consumer comuns.
