...
/** O próprio trabalhador confirma que viu o resultado da candidatura (ex: "você foi chamado"). */
export async function markApplicationSeen(workerId: string, applicationId: string): Promise<ApplicationResponse> {
  ...
  if (application.workerSeenAt) {
    return toApplicationResponse(application);
  }

  const [updated] = await db
    .update(applications)
    .set({ workerSeenAt: new Date() })
