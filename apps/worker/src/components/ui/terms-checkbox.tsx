...
export function TermsCheckbox({ checked, onChange, id = 'terms-accepted' }: TermsCheckboxProps) {
  return (
    <label htmlFor={id} className="flex items-start gap-2.5 text-sm text-text-secondary">
      ...
      />
      <span>
        Li e concordo que o Pronto é só um intermediário: não é meu empregador, não dirige nem fiscaliza o
        trabalho, e a prestação de serviço acontece diretamente entre empresa e profissional, sem vínculo
        empregatício com o Pronto.
      </span>
    </label>
  );
}
